...
def main(GetBusData: func.TimerRequest) -> func.HttpResponse:
    logging.info('GetBusData function triggered !!!=' + AZURE_CONN_STRING)
    ...
    logging.info('Received {0} buses positions, found {1} buses in monitored routes'.format(
        len(feed), len(buses)))
    ...
    for gf in activatedGeofences:
        logging.info('Vehicle %i, route %s, %sing GeoFence %s at %s UTC',
                     gf["VehicleId"], gf["RouteId"], gf["GeoFenceStatus"], gf["GeoFence"], gf["TimestampUTC"])
        TriggerLogicApp(gf)
...
def TriggerLogicApp(geoFence):
    ...
    logging.info("Calling Logic App webhook for {0}".format(
        geoFence["VehicleId"]))
    ...
    if response.status_code != 202:
        logging.info('Error calling Logic App: {0}'.format(
            response.status_code))
    else:
        logging.info("[%i/%i/%i] WebHook called successfully",
                     geoFence["VehicleId"], geoFence["DirectionId"], geoFence["GeoFenceId"])
